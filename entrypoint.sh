...
menu_minecraft_java() {
    ...
    case $option in
        ...
        5) print_forge_versions "$MC_VERSION"; read -r -p "Please choose a Forge version: " FORGE_VERSION; if ! print_forge_versions "$MC_VERSION" | grep -q "^${FORGE_VERSION}\$"; then echo "Invalid Forge version selected!"; exit 1; fi; install_forge "$MC_VERSION" "$FORGE_VERSION"; save_selection "Java" "Forge" "$MC_VERSION" "$FORGE_VERSION"; create_start_script; exit 0 ;;
        ...
        7) print_neoforge_versions "$MC_VERSION"; read -r -p "Please choose a NeoForge version: " NEOFORGE_VERSION; if ! print_neoforge_versions "$MC_VERSION" | grep -q "^${NEOFORGE_VERSION}\$"; then echo "Invalid NeoForge version selected!"; exit 1; fi; install_neoforge "$MC_VERSION" "$NEOFORGE_VERSION"; save_selection "Java" "NeoForge" "$MC_VERSION" "$NEOFORGE_VERSION"; create_start_script; exit 0 ;;
        0) main_menu ;;
